...
In Laravel, exception reporting is used to log exceptions or send them to an external service like [Sentry](https://github.com/getsentry/sentry-laravel) or [Flare](https://flareapp.io). By default, exceptions will be logged based on your [logging](/laravel/12.x/logging) configuration. However, you are free to log exceptions however you wish.
...
> [!NOTE]
> To customize the exception reporting for a given exception, you may also utilize [reportable exceptions](/laravel/12.x/errors#renderable-exceptions).
...
When messages are written to your application's [logs](/laravel/12.x/logging), the messages are written at a specified [log level](/laravel/12.x/logging#log-levels), which indicates the severity or importance of the message being logged.
...
> [!NOTE]
> You may type-hint any required dependencies of the `report` method and they will automatically be injected into the method by Laravel's [service container](/laravel/12.x/container).
