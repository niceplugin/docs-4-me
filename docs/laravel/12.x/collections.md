...
> [!NOTE]
> The results of [Eloquent](/laravel/12.x/eloquent) queries are always returned as `Collection` instances.
...
Typically, you should declare collection macros in the `boot` method of a [service provider](/laravel/12.x/providers).
...
This method is especially useful in [views](/laravel/12.x/views) when working with a grid system such as [Bootstrap](https://getbootstrap.com/docs/5.3/layout/grid/). For example, imagine you have a collection of [Eloquent](/laravel/12.x/eloquent) models you want to display in a grid:
...
> [!NOTE]
> This method's behavior is modified when using [Eloquent Collections](/laravel/12.x/eloquent-collections#method-contains).
...
> [!NOTE]
> This method's behavior is modified when using [Eloquent Collections](/laravel/12.x/eloquent-collections#method-diff).
...
> [!NOTE]
> This method's behavior is modified when using [Eloquent Collections](/laravel/12.x/eloquent-collections#method-except).
...
> [!NOTE]
> This method's behavior is modified when using [Eloquent Collections](/laravel/12.x/eloquent-collections#method-intersect).
...
> [!NOTE]
> This method's behavior is modified when using [Eloquent Collections](/laravel/12.x/eloquent-collections#method-only).
...
> [!NOTE]
> This method's behavior is modified when interacting with [Eloquent collections](/laravel/12.x/eloquent-collections#method-partition).
...
The `toArray` method converts the collection into a plain PHP `array`. If the collection's values are [Eloquent](/laravel/12.x/eloquent) models, the models will also be converted to arrays:
...
> [!NOTE]
> This method's behavior is modified when using [Eloquent Collections](/laravel/12.x/eloquent-collections#method-unique).
...
To illustrate the usage of this method, imagine an application that submits invoices from the database using a cursor. You could define a [scheduled task](/laravel/12.x/scheduling) that runs every 15 minutes and only processes invoices for a maximum of 14 minutes:
