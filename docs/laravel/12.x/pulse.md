...
For in-depth debugging of individual events, check out [Laravel Telescope](/laravel/12.x/telescope).
...
> [!NOTE]
> You may completely customize how the authenticated user is captured and retrieved by implementing the `Laravel\Pulse\Contracts\ResolvesUsers` contract and binding it in Laravel's [service container](/laravel/12.x/container#binding-a-singleton).
...
The `<livewire:pulse.slow-outgoing-requests />` card shows outgoing requests made using Laravel's [HTTP client](/laravel/12.x/http-client) that exceed the configured threshold, which is 1,000ms by default.
...
> [!NOTE]
> Pulse uses the [cache](/laravel/12.x/cache) to store restart signals, so you should verify that a cache driver is properly configured for your application before using this feature.
...
The `CacheInteractions` recorder captures information about the [cache](/laravel/12.x/cache) hits and misses occurring in your application for display on the [Cache](#cache-card) card.
...
The `SlowOutgoingRequests` recorder captures information about outgoing HTTP requests made using Laravel's [HTTP client](/laravel/12.x/http-client) that exceed the configured threshold for display on the [Slow Outgoing Requests](#slow-outgoing-requests-card) card.
...
You may customize the [database connection](/laravel/12.x/database#configuration) used by Pulse by setting the `PULSE_DB_CONNECTION` environment variable.
...
Pulse will use your default [Redis connection](/laravel/12.x/redis#configuration) by default, but you may customize this via the `PULSE_REDIS_CONNECTION` environment variable:
...
If your custom card lives within your application's code base and you are using Laravel's [Vite integration](/laravel/12.x/vite), you may update your `vite.config.js` file to include a dedicated CSS entry point for your card:
