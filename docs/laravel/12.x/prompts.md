...
Laravel Prompts is perfect for accepting user input in your [Artisan console commands](/laravel/12.x/artisan#writing-commands), but it may also be used in any command-line PHP project.
...
Alternatively, you may leverage the power of Laravel's [validator](/laravel/12.x/validation). To do so, provide an array containing the name of the attribute and the desired validation rules to the `validate` argument:
