...
In addition to scheduling closures, you may also schedule [Artisan commands](/laravel/12.x/artisan) and system commands. For example, you may use the `command` method to schedule an Artisan command using either the command's name or class.
...
The `job` method may be used to schedule a [queued job](/laravel/12.x/queues). This method provides a convenient way to schedule queued jobs without using the `call` method to define closures to queue the job:
...
The `environments` method may be used to execute tasks only on the given environments (as defined by the `APP_ENV` [environment variable](/laravel/12.x/configuration#environment-configuration)):
...
In this example, the `emails:send` [Artisan command](/laravel/12.x/artisan) will be run every minute if it is not already running. The `withoutOverlapping` method is especially useful if you have tasks that vary drastically in their execution time, preventing you from predicting exactly how long a given task will take.
...
Behind the scenes, the `withoutOverlapping` method utilizes your application's [cache](/laravel/12.x/cache) to obtain locks. If necessary, you can clear these cache locks using the `schedule:clear-cache` Artisan command. This is typically only necessary if a task becomes stuck due to an unexpected server problem.
...
Using the `emailOutputTo` method, you may email the output to an email address of your choice. Before emailing the output of a task, you should configure Laravel's [email services](/laravel/12.x/mail):
...
Laravel dispatches a variety of [events](/laravel/12.x/events) during the scheduling process. You may [define listeners](/laravel/12.x/events) for any of the following events:
