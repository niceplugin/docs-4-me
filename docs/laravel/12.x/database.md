...
Almost every modern web application interacts with a database. Laravel makes interacting with databases extremely simple across a variety of supported databases using raw SQL, a [fluent query builder](/laravel/12.x/queries), and the [Eloquent ORM](/laravel/12.x/eloquent). Currently, Laravel provides first-party support for five databases:
...
By default, Laravel's sample [environment configuration](/laravel/12.x/configuration#environment-configuration) is ready to use with [Laravel Sail](/laravel/12.x/sail), which is a Docker configuration for developing Laravel applications on your local machine. However, you are free to modify your database configuration as needed for your local database.
...
> [!NOTE]
> If you use the [Laravel installer](/laravel/12.x/installation#creating-a-laravel-project) to create your Laravel application and select SQLite as your database, Laravel will automatically create a `database/database.sqlite` file and run the default [database migrations](/laravel/12.x/migrations) for you.
...
A common performance bottleneck of modern web applications is the amount of time they spend querying databases. Thankfully, Laravel can invoke a closure or callback of your choice when it spends too much time querying the database during a single request. To get started, provide a query time threshold (in milliseconds) and closure to the `whenQueryingForLongerThan` method. You may invoke this method in the `boot` method of a [service provider](/laravel/12.x/providers):
...
> [!NOTE]
> The `DB` facade's transaction methods control the transactions for both the [query builder](/laravel/12.x/queries) and [Eloquent ORM](/laravel/12.x/eloquent).
...
To get started, you should schedule the `db:monitor` command to [run every minute](/laravel/12.x/scheduling). The command accepts the names of the database connection configurations that you wish to monitor as well as the maximum number of open connections that should be tolerated before dispatching an event:
