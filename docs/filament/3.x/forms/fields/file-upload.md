...
> 이미지 및 기타 파일을 올바르게 미리보기하려면 FilePond가 앱과 동일한 도메인에서 파일이 제공되거나, 적절한 CORS 헤더가 있어야 합니다. `APP_URL` 환경 변수가 올바른지 확인하거나, [filesystem](/laravel/12.x/filesystem) 드라이버를 수정하여 올바른 URL을 설정하세요. S3와 같은 별도의 도메인에 파일을 호스팅하는 경우 CORS 헤더가 설정되어 있는지 확인하세요.
...
> 개발자가 파일이 삭제될 때 디스크에서 해당 파일을 삭제하는 책임이 있습니다. Filament는 해당 파일이 다른 곳에서 사용되는지 알지 못합니다. 이를 자동으로 처리하는 한 가지 방법은 [모델 이벤트](/laravel/12.x/eloquent#events)를 관찰하는 것입니다.
...
Eloquent를 사용하여 파일 URL을 저장하는 경우, 모델 속성에 `array` [캐스트](/laravel/12.x/eloquent-mutators#array-and-json-casting)를 추가해야 합니다:
...
이제 `attachment_file_names`에 업로드된 파일의 원본 파일 이름이 저장되므로, 폼 제출 시 데이터베이스에 저장할 수 있습니다. `multiple()` 파일을 업로드하는 경우, 이 Eloquent 모델 속성에도 `array` [캐스트](/laravel/12.x/eloquent-mutators#array-and-json-casting)를 추가해야 합니다.
...
[최대 업로드 크기는 `temporary_file_upload`의 `rules` 키](/livewire/3.x/uploads#global-validation)에서 조정할 수 있습니다. 이때 KB 단위로 입력하며, 120MB는 122880KB입니다:
