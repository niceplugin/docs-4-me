...
### 패널 빌더 렌더 후크 {#panel-builder-render-hooks}
...
- `PanelsRenderHook::BODY_END` - `</body>` 이전
- `PanelsRenderHook::BODY_START` - `<body>` 이후
...
- `PanelsRenderHook::HEAD_END` - `</head>` 이전
- `PanelsRenderHook::HEAD_START` - `<head>` 이후
...
- `PanelsRenderHook::PAGE_SUB_NAVIGATION_SELECT_AFTER` - 페이지 서브 내비게이션 셀렉트(모바일용) 이후, [페이지 또는 리소스 클래스에 범위 지정](#scoping-render-hooks) 가능
- `PanelsRenderHook::PAGE_SUB_NAVIGATION_SELECT_BEFORE` - 페이지 서브 내비게이션 셀렉트(모바일용) 이전, [페이지 또는 리소스 클래스에 범위 지정](#scoping-render-hooks) 가능
...
- `PanelsRenderHook::SIDEBAR_NAV_END` - [사이드바](../panels/navigation)에서 `</nav>` 이전
- `PanelsRenderHook::SIDEBAR_NAV_START` - [사이드바](../panels/navigation)에서 `<nav>` 이후
...
이 모든 렌더 후크는 [범위 지정](#scoping-render-hooks)하여 테이블 Livewire 컴포넌트 클래스에 적용할 수 있습니다. 패널 빌더를 사용할 때, 이 클래스들은 리소스의 List 또는 Manage 페이지이거나 관계 매니저일 수 있습니다. 테이블 위젯도 Livewire 컴포넌트 클래스입니다.
...
- `TablesRenderHook::TOOLBAR_GROUPING_SELECTOR_AFTER` - [그룹화](../tables/grouping) 셀렉터 이후
- `TablesRenderHook::TOOLBAR_GROUPING_SELECTOR_BEFORE` - [그룹화](../tables/grouping) 셀렉터 이전
- `TablesRenderHook::TOOLBAR_REORDER_TRIGGER_AFTER` - [정렬 변경](../tables/advanced#reordering-records) 트리거 이후
- `TablesRenderHook::TOOLBAR_REORDER_TRIGGER_BEFORE` - [정렬 변경](../tables/advanced#reordering-records) 트리거 이전
- `TablesRenderHook::TOOLBAR_SEARCH_AFTER` - [검색](../tables/getting-started#making-columns-sortable-and-searchable) 컨테이너 이후
- `TablesRenderHook::TOOLBAR_SEARCH_BEFORE` - [검색](../tables/getting-started#making-columns-sortable-and-searchable) 컨테이너 이전
- `TablesRenderHook::TOOLBAR_START` - 툴바의 시작
- `TablesRenderHook::TOOLBAR_TOGGLE_COLUMN_TRIGGER_AFTER` - [컬럼 토글](../tables/columns/getting-started#toggling-column-visibility) 트리거 이후
- `TablesRenderHook::TOOLBAR_TOGGLE_COLUMN_TRIGGER_BEFORE` - [컬럼 토글](../tables/columns/getting-started#toggling-column-visibility) 트리거 이전


### 위젯 렌더 후크 {#widgets-render-hooks}

```php
    use Filament\Widgets\View\WidgetsRenderHook;
```

- `WidgetsRenderHook::TABLE_WIDGET_END` - [테이블 위젯](../panels/dashboard#table-widgets)의 끝, 테이블 자체 이후, [테이블 위젯 클래스에 범위 지정](#scoping-render-hooks) 가능
- `WidgetsRenderHook::TABLE_WIDGET_START` - [테이블 위젯](../panels/dashboard#table-widgets)의 시작, 테이블 자체 이전, [테이블 위젯 클래스에 범위 지정](#scoping-render-hooks) 가능


## 렌더 후크 범위 지정하기 {#scoping-render-hooks}
...
[패널 빌더](#panel-builder-render-hooks)의 일부 렌더 후크는 리소스의 모든 페이지에 후크를 범위 지정할 수 있습니다:
...
### 렌더 후크 내부에서 현재 활성화된 범위 가져오기 {#retrieving-the-currently-active-scopes-inside-the-render-hook}

`$scopes`는 렌더 후크 함수에 전달되며, 이를 사용하여 렌더 후크가 어느 페이지나 컴포넌트에서 렌더링되고 있는지 확인할 수 있습니다:
...
렌더 후크에 범위를 제공하려면, `renderHook()`의 두 번째 인자로 전달할 수 있습니다. 예를 들어, 후크가 Livewire 컴포넌트 내부에 있다면, `static::class`를 사용하여 컴포넌트의 클래스를 전달할 수 있습니다:
